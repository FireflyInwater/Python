...
def creat_clusters(Another_point,K_point):
    ...
    for point in K_point:
        ...
        if dis_between_Kpoint <= dis_exchange[0]:  # 取最小距离分类
            ...
            dis_exchange[1] = point[1]  #取标签
    return dis_exchange[1]  #返回这个样本点归类标签

def get_Kdiffirent_point(N,K,Labels):
    """生成K个随机索引,对于K个不同类别的质心"""
    while True:
        random_index = random.sample(range(0, N.shape[0]), K)  # 生成K个随机索引,对于K个不同类别的质心
...
def recent_calssfiy(N,K_point):
    """对剩余的每个文档测量其到每个质心的距离，把它归到最近的类"""
    cluster = {}  # 创建字典根据标签存储不同类别的样本
    for Another_point in N:
        classlabel = creat_clusters(Another_point, K_point)  # 获取每个样本分类后的标签
        if classlabel not in cluster.keys():
            cluster[classlabel] = []
            cluster[classlabel].append(Another_point)  # 根据标签归类
        else:
            cluster[classlabel].append(Another_point)
    return cluster
...
def K_Clustering(N,K,Labels):
    ...
    for i in range(max_interations):
        cluster = recent_calssfiy(N, K_point)
